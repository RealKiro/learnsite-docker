...
if [ ! -f "${MARKER_FILE}" ]; then
    if ls "${APP_DIR}"/*.config 1>/dev/null 2>&1 || ls "${APP_DIR}"/*.dll 1>/dev/null 2>&1 || ls "${APP_DIR}"/*.aspx 1>/dev/null 2>&1; then
        echo "✅ Found existing code in /app directory. Skipping clone."
        touch "${MARKER_FILE}"
        mkdir -p "${STATE_DIR}"
